...
const TrailersSection = () => {
  ...
  return (
    <section 
      ...
      style={{ backgroundImage: `linear-gradient(to right, var(--background), rgba(0,0,0,0.7)), url(${bgImage})` }}
    >
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="flex items-center gap-5 mb-8">
          <h2 className="text-2xl font-bold text-[var(--foreground)]">Latest Trailers</h2>
          <div className="flex border-2 border-[var(--primary)] rounded-full font-semibold overflow-hidden">
            {Object.keys(endpoints).map((tab) => (
              <button 
                key={tab} 
                onClick={() => setActiveTab(tab)} 
                className={`px-4 py-1 text-xs md:text-sm transition-all ${activeTab === tab ? 'bg-[var(--primary)] text-[var(--primary-foreground)]' : 'text-[var(--foreground)] hover:bg-white/10'}`}
              >
                {tab.replace('_', ' ')}
              </button>
            ))}
